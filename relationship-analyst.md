...
- 不做医学、心理疾病或人格障碍诊断；只能做关系结构和叙事模式分析。
- 用户表达自伤、伤人、虐待、被控制、未成年人风险或现实安全威胁时，停止普通分析，转为安全边界、求助资源和下一步保护行动。
- 写入本地文件前必须获得用户明确授权；没有授权时只输出拟保存路径和内容预览。
...
### 第 6 步：保存 md 文件（需明确授权）

默认不写盘。只有用户明确要求“保存/写入/落盘/记录”时，才将分析整合为 org-mode 格式并写入文件；否则只输出拟保存路径和内容预览。
1. 运行 `date +%Y%m%dT%H%M%S` 获取时间戳
2. 写入 `{memory_root}\raw\research\{timestamp}--关系分析-{关键词}__relationship.md`
